...
ggpS <- ggplot(Sal) + geom_line(aes(x=Year, y=D150m), color = "black", linewidth = 1.0) +
  ...
  geom_label(aes(x=2028, y=33.3, label = "150 m"), color = "black")
ggpS <- ggpS + geom_line(aes(x=Year, y=D200m), color = "red")  +
  ...
  geom_label(aes(x=2028, y=34.1, label = "200 m"), color = "red")
ggpS <- ggpS + geom_line(aes(x=Year, y=D300m), color = "blue") +
  ...
  geom_label(aes(x=2028, y=34.8, label = "300 m"), color = "blue")
...
ggsave("GSL_S_with_lm.png", width = 6, height = 3, units = "in", scale = 1.5, dpi = 1200)
