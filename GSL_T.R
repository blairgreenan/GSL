...
ggpT <- ggplot(Tem) + geom_line(aes(x=Year, y=D150m), color = "black", linewidth = 1.0) +
  ...
  geom_label(aes(x=2027, y=2.8, label = "150 m"), color = "black")
ggpT <- ggpT + geom_line(aes(x=Year, y=D200m), color = "red")  +
  ...
  geom_label(aes(x=2027, y=5.1, label = "200 m"), color = "red")
ggpT <- ggpT + geom_line(aes(x=Year, y=D300m), color = "blue") +
  ...
  geom_label(aes(x=2027, y=7.6, label = "300 m"), color = "blue")
...
ggsave("GSL_T_with_lm.png", width = 6, height = 3, units = "in", scale = 1.0, dpi = 1200)
